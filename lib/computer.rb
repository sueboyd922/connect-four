...
class Computer
  #a computers is initialized with a board, same as player's board
  ...
  def random_letter
    sleep([0, 1].sample) #neat trick Kim and Joseph shared, the computer will pause for 0-2 seconds as if it's thinking. It's obviously not, this computer is stupid.
    @computer_input = @board.valid_columns.keys.sample # gets a random letter A-G from the column hash keys
    validate(@computer_input) #checks to see if it's a playable column
    @computer_input #this is here to force a test to pass...
  end

  # method to determine if the spot is playable
  def validate(computer_input)
